  ...
  return (
    <section className="py-20 px-4 relative overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-primary/5 via-secondary/3 to-accent/5" style={{ zIndex: 0 }}>
        ...
        >
          <div className="text-primary/60 font-mono text-sm whitespace-nowrap">
            {"const developer = { name: 'Mohammad Khalid', skills: ['React', 'Node.js', 'Python'], passion: 'Security' };"}
          </div>
        </motion.div>
        
        <motion.div 
          className="absolute top-1/2 right-0 w-full h-20 opacity-30"
          animate={{ x: [1400, -100] }}
          transition={{ duration: 18, repeat: Infinity, ease: "linear", delay: 2 }}
        >
          <div className="text-secondary/60 font-mono text-sm whitespace-nowrap text-right">
            {"function secureFuture() { return innovation + ethics + technology; }"}
          </div>
        </motion.div>
        
        <motion.div 
          className="absolute top-3/4 left-0 w-full h-20 opacity-30"
          animate={{ x: [-100, 1400] }}
          transition={{ duration: 20, repeat: Infinity, ease: "linear", delay: 5 }}
        >
          <div className="text-accent/60 font-mono text-sm whitespace-nowrap">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        >
          <motion.h2 
            className="text-5xl md:text-6xl font-bold mb-4 bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent hover:scale-105 transition-transform duration-300"
          ...
          />
          <motion.p 
            className="text-xl text-muted-foreground max-w-3xl mx-auto hover:text-foreground transition-colors duration-300"
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            >
              <motion.p 
                className="text-lg text-foreground leading-relaxed hover:scale-105 transition-transform duration-300"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.3, delay: 0.3 }}
              >
                I am a passionate tech enthusiast currently pursuing my B.Tech degree (3rd Year) at 
                <span className="font-semibold text-primary bg-primary/10 px-2 py-1 rounded-md mx-1 hover:bg-primary/20 transition-colors duration-300">
            ...
            >
              {[
                ...
                { number: "20+", label: "Projects", delay: 0.1 },
                { number: "5+", label: "Certifications", delay: 0.2 }
              ].map((stat, index) => (
                <motion.div
                  key={stat.label}
                  className="text-center p-6 bg-card rounded-xl shadow-lg border border-border hover:scale-110 hover:shadow-xl hover:-translate-y-2 transition-all duration-300 cursor-pointer"
                  initial={{ opacity: 0, scale: 0.8 }}
                  whileInView={{ opacity: 1, scale: 1 }}
                  whileHover={{ scale: 1.1, y: -8 }}
                  transition={{ duration: 0.3, delay: 0.6 + stat.delay }}
                >
                  <motion.div 
                    className="text-4xl font-bold text-primary mb-2"
                    initial={{ scale: 0 }}
                    whileInView={{ scale: 1 }}
                    transition={{ duration: 0.3, delay: 0.7 + stat.delay, type: "spring", stiffness: 200 }}
                  >
                    {stat.number}
                  </motion.div>
            ...
            >
              {[
                ...
                { icon: Users, label: "Team Leader", delay: 0.05 },
                { icon: Target, label: "Problem Solver", delay: 0.1 }
              ].map((badge, index) => (
                <motion.div
                  key={badge.label}
                  className="flex items-center gap-2 bg-primary/10 text-primary px-4 py-2 rounded-full text-sm font-medium border border-primary/20 hover:bg-primary/20 hover:scale-110 transition-all duration-300 cursor-pointer"
                  initial={{ opacity: 0, scale: 0.8 }}
                  whileInView={{ opacity: 1, scale: 1 }}
                  whileHover={{ scale: 1.1, y: -2 }}
                  transition={{ duration: 0.25, delay: 0.9 + badge.delay }}
                >
          ...
          >
            {[
              ...
              <motion.div
                key={item.title}
                className="bg-card p-8 rounded-2xl shadow-xl border border-border hover:shadow-2xl hover:scale-105 hover:-translate-y-3 transition-all duration-500 cursor-pointer group"
              ...
              >
                <div className="flex items-center mb-6">
                  <motion.div 
                    className={`w-16 h-16 bg-gradient-to-br ${item.gradient} rounded-2xl flex items-center justify-center mr-4 shadow-lg group-hover:scale-110 group-hover:rotate-6 transition-all duration-300`}
                    initial={{ scale: 0, rotate: -90 }}
                    whileInView={{ scale: 1, rotate: 0 }}
                    whileHover={{ scale: 1.2, rotate: 12 }}
                    transition={{ duration: 0.3, delay: 0.4 + item.delay, type: "spring", stiffness: 200 }}
                  >
                  ...
                  <div>
                    ...
                    <motion.p 
                      className="text-muted-foreground font-medium group-hover:text-secondary transition-colors duration-300"
                      initial={{ opacity: 0, x: -20 }}
                      whileInView={{ opacity: 1, x: 0 }}
                      transition={{ duration: 0.3, delay: 0.55 + item.delay }}
                    >
                      {item.subtitle}
                    </motion.p>
                  </div>
                ...
                <motion.p 
                  ...
                  whileInView={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.3, delay: 0.6 + item.delay }}
                >
                  {item.description}
                </motion.p>
              </motion.div>
